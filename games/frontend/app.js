...
const ASSET_VERSION = "20260729r5";
const LOGO_ASSETS = ["logo.png", "logo.svg"];
...
function logoAssetUrl(index = 0) {
  const asset = LOGO_ASSETS[index] || LOGO_ASSETS[LOGO_ASSETS.length - 1];
...
function renderQrFallback(host, text, size, fallbackMessage) {
  host.innerHTML = "";
  try {
    if (typeof window.QRCode === "function") {
      ...
      const rendered = !!qs("canvas, img", host);
      reportDebug("B", "QR image fallback to local library", { text, size, rendered });
      return rendered;
    }
  } catch {}
...
async function fetchQrDataUrl(text, size = 280) {
  const res = await fetch(qrImageUrl(text, size), {
    cache: "no-store",
...
async function renderQrNode(host, text, size = 280, fallbackMessage = "QR generation unavailable") {
  if (!host) {
    // #region debug-point B:qr-missing-lib
    reportDebug("B", "QRCode library unavailable during render", {
      hasHost: !!host,
  ...
  try {
    ...
    qrImg.loading = "eager";
    qrImg.alt = "QR code";
    qrImg.src = await fetchQrDataUrl(text, size);
    await waitForImage(qrImg);
    host.appendChild(qrImg);
    // #region debug-point B:qr-render-success
    reportDebug("B", "QR rendered", {
      text,
      size,
      hasImg: !!qrImg,
      srcPrefix: qrImg?.src?.slice(0, 30) || "",
    });
    // #endregion
    return true;
  } catch (e) {
    try {
      return renderQrFallback(host, text, size, fallbackMessage);
    } catch {}
    // #region debug-point B:qr-render-failed
    reportDebug("B", "QR render failed", { text, size, message: e?.message || "unknown" });
    // #endregion
    host.replaceWith(el(`<div class="hint bad">${escapeHtml(fallbackMessage)}</div>`));
    return false;
  }
...
function hydrateLogoImages(root = document) {
  root.querySelectorAll("[data-logo]").forEach((img) => {
    if (!(img instanceof HTMLImageElement)) return;
    const primary = logoAssetUrl(0);
    const fallback = logoAssetUrl(1);
    img.onerror = () => {
      ...
      <div class="topbar">
        <div class="topbar-inner">
          <a class="brand" href="/coach">
            <img class="brand-mark" data-logo alt="CJM Global Integrity Hub logo" />
...
function ensureRealtime() {
  ...
  // #region debug-point C:realtime-init
  reportDebug("C", "Realtime listeners ensured", {
    hasSse: !!sse,
    hasPollTimer: !!pollTimer,
    phase: publicState?.phase || null,
  });
  // #endregion
}

function renderLeaderboardIfVisible() {
  const table = arguments[0] ? qs('[data-role="leaderboard-table"]', arguments[0]) : qs('[data-role="leaderboard-table"]');
  if (!table) return;
  const tbody = qs("tbody", table);
  if (!tbody) return;
  const rows = leaderboard?.rows || [];
  // #region debug-point E:leaderboard-render
  reportDebug("E", "Rendering leaderboard rows", {
    rowCount: rows.length,
    rows: rows.map((r) => ({
      rank: r.rank,
      teamName: r.teamName,
      completedMissions: r.completedMissions,
      currentMissionName: r.currentMissionName,
    })),
  });
  // #endregion
  ...
  const node = viewCard(
    ...
    `
      <div class="hero-logo-wrap">
        <img class="hero-logo" data-logo alt="CJM Global Integrity Hub logo" />
      </div>
      <div class="hint">
...
function renderConfig() {
  ...
  function updateUi() {
    ...
    // #region debug-point D:config-selection
    reportDebug("D", "Config selection updated", {
      count,
      max,
...
function renderRegistration() {
  ...
  const url = joinUrl();
  // #region debug-point C:registration-view
  reportDebug("C", "Rendering coach registration view", {
    teamCount: teams.length,
    registrationOpen: publicState?.registrationOpen,
  ...
  renderQrNode(qrHost, url, 280, "Registration QR failed to render.").catch(() => {});
...
function renderLeaderboard() {
  ...
  renderLeaderboardIfVisible(node);
...
async function renderPrint() {
  ...
  const settled = await Promise.all(renderResults);
  const allReady = settled.every(Boolean);
