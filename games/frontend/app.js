...
// #region debug-point A:report-runtime-state
function reportDebug(hypothesisId, msg, data = {}, runId = "pre-fix") {
  fetch("http://127.0.0.1:7777/event", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      sessionId: "blank-qr-logo",
      runId,
      hypothesisId,
...
function qrImageUrl(text, size = 0) {
...
async function primeQrImage(img, text, options = {}) {
  if (!img) return false;
  const size = Number(options.size || 0) || 0;
  const hypothesisId = options.hypothesisId || "B";
  const fallbackMessage = options.fallbackMessage || "QR generation unavailable";
  try {
    const res = await fetch(qrImageUrl(text, size), { cache: "no-store" });
    ...
    reportDebug(hypothesisId, "QR preload failed", {
      text,
      size,
      message: e?.message || "unknown",
    });
    img.replaceWith(el(`<div class="hint bad">${escapeHtml(fallbackMessage)}</div>`));
    return false;
  }
}
      ...
      <div class="topbar">
        <div class="topbar-inner">
          <a class="brand" href="/coach">
            <img class="brand-mark" src="/assets/logo.svg" alt="CJM Global Integrity Hub logo" />
  ...
  const node = viewCard(
    ...
    `
      <div class="hero-logo-wrap">
        <img class="hero-logo" src="/assets/logo.svg" alt="CJM Global Integrity Hub logo" />
      </div>
      <div class="hint">
...
function renderConfig() {
    ...
    `
      <div class="grid">
        ...
        <div class="footer-note">
          Rules: Coach can select up to ${max} games. You may continue with fewer than ${max}.
        </div>
      </div>
    `
  );
  ...
  function updateUi() {
    ...
    let kind = count > 0 ? "good" : "";
    if (count === 0) msg += " (select at least 1)";
    if (count > max) {
...
function renderRegistration() {
      ...
      <div class="split">
        <div class="card" style="box-shadow:none;background:transparent;border:none;">
          <div class="card-inner" style="padding:0;">
            <div class="qr-box">
              <img data-role="reg-qr" alt="Registration QR code" />
            </div>
  ...
  const qrImage = qs('[data-role="reg-qr"]', node);
  // #region debug-point A:registration-qr
  reportDebug("A", "Attempting registration QR render", {
    hasQRCodeGlobal: !!window.QRCode,
    imagePresent: !!qrImage,
    url,
  });
  // #endregion
  if (qrImage) {
    ...
    qrImage.addEventListener(
      "error",
      () => {
        qrImage.replaceWith(el(`<div class="hint bad">QR generation unavailable</div>`));
      },
      { once: true }
    );
  }
...
function createScanner(onText) {
  ...
  const host = el(`
    <div class="grid">
      <div class="hint">
        ...
        <span>${canDetect ? "Allow camera access and point at the QR." : "Scanner not supported. Use manual entry below."}</span>
      </div>
      <div class="scanner" data-role="scanner"></div>
      <div class="field">
        <div class="label">Manual QR Content</div>
        <input type="text" placeholder="IH|GAME|integrity_mat" data-role="manual" />
      </div>
      <div class="btn-row">
        <button class="primary" data-action="submit-manual">SUBMIT</button>
        <button data-action="stop">STOP</button>
      </div>
      <div class="footer-note">If your phone browser does not support in-app scanning, you can still submit the QR text manually.</div>
    </div>
  `);
  ...
  const manual = qs('[data-role="manual"]', host);
  const submitManual = qs('[data-action="submit-manual"]', host);
  ...
  submitManual.addEventListener("click", () => {
    ...
    if (!v) {
      toast("Please paste the QR content.", "bad");
      return;
    }
    if (scanBusy) return;
    scanBusy = true;
    submitManual.disabled = true;
    onText(v);
    manual.value = "";
    setTimeout(() => {
      scanBusy = false;
      submitManual.disabled = false;
    }, 800);
  });
  ...
  setTimeout(() => startCamera().catch(() => {}), 10);
  return { host, stop };
}
...
async function renderPrint() {
  ...
  const reg = el(`
    <div class="card">
      <div class="card-inner">
        ...
        <div class="qr-box"><img data-role="qr" alt="Registration QR code for participants" /></div>
  ...
  // #region debug-point B:print-registration-qr
  reportDebug("B", "Attempting printable registration QR render", {
    hasQRCodeGlobal: !!window.QRCode,
    imagePresent: !!regQr,
    url: joinUrl(),
  });
  ...
  if (regQr) {
    ...
    regQr.height = 260;
    preloadTasks.push(
      primeQrImage(regQr, joinUrl(), {
        size: 900,
        hypothesisId: "B",
        fallbackMessage: "Registration QR failed to load.",
      })
    );
  }
  ...
  all.forEach((g) => {
    ...
    const card = el(`
      <div class="card">
        <div class="card-inner">
          ...
          <div class="qr-box"><img data-role="qr" alt="${escapeHtml(g.name)} station QR code" /></div>
    ...
    // #region debug-point B:print-station-qr
    reportDebug("B", "Attempting printable station QR render", {
      hasQRCodeGlobal: !!window.QRCode,
      imagePresent: !!c,
      payload,
      gameId: g.id,
    ...
    if (c) {
      ...
      c.height = 260;
      preloadTasks.push(
        primeQrImage(c, payload, {
          size: 900,
          hypothesisId: "B",
          fallbackMessage: `${g.name} QR failed to load.`,
        })
      );
    }
  });
