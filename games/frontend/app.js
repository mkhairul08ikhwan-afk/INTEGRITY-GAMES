...
async function fetchJson(url, options) {
  ...
  if (!res.ok) {
    const msg = (data && (data.description || data.message)) || `${res.status} ${res.statusText}`;
...
async function ensurePublicState() {
  ...
  publicState = await fetchJson(API.public);
}
...
function connectStream() {
  ...
  sse.addEventListener("state", (ev) => {
    try {
      publicState = JSON.parse(ev.data);
    } catch {
      return;
    }
    render();
...
function renderRegistration() {
      ...
      <div class="split">
        <div class="card" style="box-shadow:none;background:transparent;border:none;">
          <div class="card-inner" style="padding:0;">
            <div class="qr-box">
              <canvas width="280" height="280" data-role="reg-qr"></canvas>
            </div>
  ...
  const canvas = qs('[data-role="reg-qr"]', node);
  try {
    if (window.QRCode && canvas) {
      window.QRCode.toCanvas(canvas, url, { width: 280, margin: 1 });
    }
  } catch {
    if (canvas) canvas.replaceWith(el(`<div class="hint bad">QR generation unavailable</div>`));
  }
...
async function renderParticipantDashboard(team) {
  ...
  async function handleScanText(text) {
    ...
    try {
      ...
      if (res.ok === false && res.result === "wrong_mission") {
        const expected = res.expectedName || "your current mission";
        ...
        toast("Wrong mission scanned.", "bad");
      } else {
...
function renderPrint() {
  ...
  const reg = el(`
    <div class="card">
      <div class="card-inner">
        ...
        <div class="qr-box"><canvas width="260" height="260" data-role="qr"></canvas></div>
  ...
  try {
    const c = qs('[data-role="qr"]', reg);
    if (window.QRCode && c) window.QRCode.toCanvas(c, joinUrl(), { width: 260, margin: 1 });
  } catch {}
  ...
  all.forEach((g) => {
    ...
    try {
      ...
      if (window.QRCode && c) window.QRCode.toCanvas(c, payload, { width: 260, margin: 1 });
    } catch {}
  });
