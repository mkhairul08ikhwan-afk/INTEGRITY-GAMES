...
def _validate_selected_games(selected: Any) -> List[str]:
    ...
    if not selected:
        _raise_api_error(400, "invalid_selected_games", "Coach must select at least 1 game.")
    if len(selected) > MAX_SELECTED_GAMES:
        _raise_api_error(400, "invalid_selected_games", f"Coach can select a maximum of {MAX_SELECTED_GAMES} games.")
    normalized: List[str] = []
    ...
    if not unique_ids:
        _raise_api_error(400, "invalid_selected_games", "Coach must select at least 1 unique game.")
    if len(unique_ids) > MAX_SELECTED_GAMES:
        _raise_api_error(400, "invalid_selected_games", f"A maximum of {MAX_SELECTED_GAMES} unique games is allowed.")
    return unique_ids
