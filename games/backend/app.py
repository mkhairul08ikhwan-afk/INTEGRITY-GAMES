...
def _build_qr_png(text: str, target_size: Optional[int] = None) -> bytes:
    border = 4
    base_qr = qrcode.QRCode(
        version=None,
        error_correction=qrcode.constants.ERROR_CORRECT_H,
        box_size=1,
        border=border,
    )
    base_qr.add_data(text)
    base_qr.make(fit=True)
    module_span = base_qr.modules_count + (border * 2)
    box_size = 12
    if target_size:
        box_size = max(1, target_size // module_span)
    qr = qrcode.QRCode(
        version=base_qr.version,
        error_correction=qrcode.constants.ERROR_CORRECT_H,
        box_size=box_size,
        border=border,
    )
...
def _validate_selected_games(selected: Any) -> List[str]:
    ...
    if len(unique_ids) != MAX_SELECTED_GAMES:
        _raise_api_error(400, "invalid_selected_games", f"Exactly {MAX_SELECTED_GAMES} unique games are required.")
    return unique_ids
...
def create_app() -> Flask:
    ...
    def after_request(response: Response) -> Response:
        ...
        if path.startswith("/api/"):
            ...
            response.headers["Cache-Control"] = f"public, max-age={ASSET_MAX_AGE_SECONDS}, must-revalidate"
        else:
    ...
    def api_start() -> Response:
        with state_lock:
            ...
            if len(state.selected_games) != MAX_SELECTED_GAMES:
                _raise_api_error(409, "games_not_configured", "Games are not configured.")
    ...
    def api_qr() -> Response:
        ...
        if not text:
            return api_error("validation_error", "Query parameter 'text' is required.", 400)
        if len(text) > 1024:
            return api_error("validation_error", "QR content is too long.", 400)
        target_size: Optional[int] = None
        if size_raw:
            try:
                target_size = int(size_raw)
            except ValueError:
                return api_error("validation_error", "Query parameter 'size' must be an integer.", 400)
            if target_size < 128 or target_size > 2048:
                return api_error("validation_error", "Query parameter 'size' must be between 128 and 2048.", 400)
        try:
            png_bytes = _build_qr_png(text, target_size=target_size)
        except Exception:
            LOGGER.exception("qr_render_failed")
            return api_error("qr_render_failed", "Failed to generate QR code.", 500)
